...
import lishui.study.util.SingleManager;
...
//		doNotLeakWork();
	...
	private void doLeakWork() {
		...
		new Handler().postDelayed(new Runnable() {
			@Override
			public void run() {
				LogUtil.d(TAG, Thread.currentThread().getName() + ": doing a delay work but leak now");
			}
		}, 1000 * 60 * 60);
			...
			if (leakActivityWeakReference != null && leakActivityWeakReference.get() != null){
				switch (msg.what){
					case NOT_LEAK_HANDLER_MSG:
						LogUtil.d(TAG, Thread.currentThread().getName() + ": doing a delay work but not leak. ");
						break;
				}
			}
		}
